...
<?php
    ...
    if ($salons)
    {
        while ($salon = mysqli_fetch_assoc($salons))
        {
            $salon_list .= "<tr>";
            $salon_list .= "<td> {$salon['salon_id']} </td>";
            $salon_list .= "<td> {$salon['salon_name']} </td>";
            $salon_list .= "<td> {$salon['salon_city']} </td>";
            $salon_list .= "<td> {$salon['salon_address']} </td>";
            $salon_list .= "<td> {$salon['salon_contact_no']} </td>";
            $salon_list .= "<td> {$salon['salon_email']} </td>";
            $salon_list .= "<td> {$salon['salon_note']} </td>";
            $salon_list .= "<td> <a href=\"update_salons.php?salon_id={$salon['salon_id']}\"> Update </a></td>";
            $salon_list .= "<td> <a href=\"delete_salons.php?salon_id={$salon['salon_id']}\" onclick=\"return confirm('Do you want delete this salon?');\"> Delete </a></td>";
            $salon_list .= "</tr>";
        }
    }
    else
...
<html>
    <head>
        ...
        <title> Manage Salons - SalonBooking.lk </title>
    ...
    <body>
        ...
        <div class="section01" style="height: 1200px; background-color: #f3eded;">
            ...
            <div class="section01-div" style="margin: 0 auto; max-width: 1500px; text-align: center;">
                ...
                <p style="font-size: 30px;font-weight: bold; color: #1d2434;"> Our Salons </p> <br><br>

                <form action="manage_salons.php" method="post">

                    <button type="button" onclick="document.location='add_salons.php'" style="background-color: #DC143C;"> Add Salon </button>
