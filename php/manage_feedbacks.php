...
<?php
    ...
    if ($feedbacks)
    {
        ...
        {
            $feedback_list .= "<tr>";
            $feedback_list .= "<td> {$feedback['feedback_id']} </td>";
            $feedback_list .= "<td> {$feedback['feedback_type']} </td>";
            $feedback_list .= "<td> {$feedback['first_name']} </td>";
            $feedback_list .= "<td> {$feedback['last_name']} </td>";
            $feedback_list .= "<td> {$feedback['email']} </td>";
            $feedback_list .= "<td> {$feedback['feedback_discription']} </td>";
            $feedback_list .= "<td> <a href=\"delete_feedbacks.php?feedback_id={$feedback['feedback_id']}\" onclick=\"return confirm('Do you want delete this feedback?');\"> Delete </a></td>";
            $feedback_list .= "</tr>";
        }
    }
    else
    ...
    if (isset($_POST['comments'])) 
    {
        ...
        $query = "SELECT * FROM feedback WHERE feedback_type = 'Comments'";
        $feedbacks = mysqli_query($conn, $query);
    ...
    else if (isset($_POST['suggestion'])) 
    {
        ...
        $query = "SELECT * FROM feedback WHERE feedback_type = 'Suggestion'";
        $feedbacks = mysqli_query($conn, $query);
    ...
    else if (isset($_POST['question'])) 
    {
        ...
        $query = "SELECT * FROM feedback WHERE feedback_type = 'Question'";
        $feedbacks = mysqli_query($conn, $query);
    ...
    // Get All Feedbacks
    else if (isset($_POST['all_feedbacks'])) 
    {
        $feedback_list = '';
...
<html>
    <head>
        ...
        <title> Manage Feedbacks - SalonBooking.lk </title>
    ...
    <body>
        ...
        <div class="section01" style="height: 1000px; background-color: #f3eded;">

            <div class="section01-div" style="margin: 0 auto; max-width: 1500px; text-align: center;">

                <p style="font-size: 30px;font-weight: bold; color: #1d2434;"> Feedbacks </p> <br><br>

                <form action="manage_feedbacks.php" method="post">

                    <button name="all_feedbacks"> All Feedbacks </button>
                    <button name="comments"> Comments </button>
                    <button name="suggestion"> Suggestion </button>
                    <button name="question"> Question </button>
