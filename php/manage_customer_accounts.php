...
<?php
    ...
    if ($customers)
    {
        ...
        {
            $customer_list .= "<tr>";
            $customer_list .= "<td> {$customer['user_id']} </td>";
            $customer_list .= "<td> {$customer['user_name']} </td>";
            $customer_list .= "<td> {$customer['first_name']} </td>";
            $customer_list .= "<td> {$customer['last_name']} </td>";
            $customer_list .= "<td> {$customer['dob']} </td>";
            $customer_list .= "<td> {$customer['gender']} </td>";
            $customer_list .= "<td> {$customer['email']} </td>";
            $customer_list .= "<td> {$customer['mobile_no']} </td>";
            $customer_list .= "<td> {$customer['city']} </td>";
            $customer_list .= "<td> <a href=\"update_customers.php?user_id={$customer['user_id']}\"> Update </a></td>";
            $customer_list .= "<td> <a href=\"delete_customers.php?user_id={$customer['user_id']}\" onclick=\"return confirm('Do you want delete this customer?');\"> Delete </a></td>";
            $customer_list .= "</tr>";
        }
    }
    else
...
<html>
    <head>
        ...
        <title> Manage Customers - SalonBooking.lk </title>
    ...
    <body>
        ...
        <div class="section01" style="height: 1200px; background-color: #f3eded;">
            ...
            <div class="section01-div" style="margin: 0 auto; max-width: 1500px; text-align: center;">
                ...
                <p style="font-size: 30px;font-weight: bold; color: #1d2434;"> Our Customers </p> <br><br>

                <form action="manage_customer_accounts.php" method="post">

                    <button type="button" onclick="document.location='add_customers.php'" style="background-color: #DC143C;"> Add Customer </button>
