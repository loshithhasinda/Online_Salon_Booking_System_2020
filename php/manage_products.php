...
<?php
    ...
    if ($products)
    {
        ...
        {
            $product_list .= "<tr>";
            $product_list .= "<td> {$item['item_id']} </td>";
            $product_list .= "<td> {$item['item_category']} </td>";
            $product_list .= "<td> {$item['item_name']} </td>";
            $product_list .= "<td> {$item['item_price']} </td>";
            $product_list .= "<td> <a href=\"update_products.php?item_id={$item['item_id']}\"> Update </a></td>";
            $product_list .= "<td> <a href=\"delete_products.php?item_id={$item['item_id']}\" onclick=\"return confirm('Do you want delete this product?');\"> Delete </a></td>";
            $product_list .= "</tr>";
        }
    }
    else
    ...
    // Get Beauty Tools Product Table
    if (isset($_POST['category02'])) 
    {
        $product_list = '';
    ...
    // Get Skin Cares Product Table
    else if (isset($_POST['category06'])) 
    {
        $product_list = '';
    ...
    // Get All Product Table
    else if (isset($_POST['category01'])) 
    {
        $product_list = '';
        $query = "SELECT * FROM product";
        $products = mysqli_query($conn, $query);
...
<html>
    <head>
        ...
        <title> Manage Products - SalonBooking.lk </title>
    ...
    <body>
        ...
        <div class="section01" style="height: 1200px; background-color: #f3eded;">
            ...
            <div class="section01-div" style="margin: 0 auto; max-width: 1500px; text-align: center;">
                ...
                <p style="font-size: 30px;font-weight: bold; color: #1d2434;"> Our Products </p> <br><br>

                <form action="manage_products.php" method="post">

                    <button name="category01"> All Products </button>
                    <button name="category02"> Beauty Toools </button>
                    <button name="category03"> Hair Cares </button>
                    <button name="category04"> Makeups </button>
                    <button name="category05"> Men's Cares </button>
                    <button name="category06"> Skin Cares </button>
                    <button type="button" onclick="document.location='add_products.php'" style="background-color: #DC143C;"> Add Product </button>
